...
class plxAdmin extends plxMotor {
	...
	public function editConfiguration($global,$content) {
		...
		if(plxUtils::write($xml,PLX_CONF))
			...
			return plxMsg::Error(L_SAVE_ERR.' '.PLX_CONF);
	...
	public function editUsers($content, $action=false) {
		...
		if($action) {
			...
			if(plxUtils::write($xml,PLX_ROOT.$this->aConf['users']))
				...
				$this->aUsers = $save;
				return plxMsg::Error(L_SAVE_ERR.' '.PLX_ROOT.$this->aConf['users']);
			}
		}
	}
	...
	public function editCategories($content, $action=false) {
		...
		if($action) {
			...
			if(plxUtils::write($xml,PLX_ROOT.$this->aConf['categories']))
				...
				$this->aCats = $save;
				return plxMsg::Error(L_SAVE_ERR.' '.PLX_ROOT.$this->aConf['categories']);
			}
		}
	}
	...
	public function editStatiques($content, $action=false) {
		...
		if($action) {
			...
			if(plxUtils::write($xml,PLX_ROOT.$this->aConf['statiques']))
				...
				$this->aStats = $save;
				return plxMsg::Error(L_SAVE_ERR.' '.PLX_ROOT.$this->aConf['statiques']);
			}
		}
	}
