...
/**
 * Classe plxUpdater responsable du gestionnaire des mises Ã  jour
 *
 * @package PLX
 * @author	Stephane F
 **/
...
class plxAdminUpdater extends plxAdmin {

	public function __construct() {
		parent::__construct(PLX_CONF);
	}
...
class plxUpdater {
	 ...
	 *
	 * @param	versions	array	liste des versions + script de mise Ã  jour (fichier versions.php)
	 * @return	null
	 * @author	Stephane F
	 **/
				...
				while($next AND !$errors) {
					...
					if(method_exists($class_name, $method_name)) {
						if(!$class_update->$method_name($this->plxAdmin)) {
							...
							$this->plxAdmin = new plxadminUpdater();
						}
					}
					else $next = false;
				}
